...
class GameLogic {
  ...
  void moveDown() {
    ...
    if (canMove(currentTetro, dy: 1)) {
      ...
      // 🔥 應用分數加成（惡魔方塊系統）
      final points = (softDropPoints * gameState.scoreMultiplier).round();
      gameState.score += points;
  ...
  void hardDrop() {
    ...
    // 🔥 應用分數加成（惡魔方塊系統）
    final points = (hardDropPoints * gameState.scoreMultiplier).round();
    gameState.score += points;
  ...
  /// 檢查是否應該觸發惡魔方塊
  void _checkDemonSpawn() {
    debugPrint(
        '[GameLogic] _checkDemonSpawn called with score=${gameState.score}');

    if (gameState.demonSpawnManager.shouldSpawn(gameState.score)) {
      debugPrint('');
      debugPrint('═══════════════════════════════════════════');
      debugPrint('🔥🔥🔥 DEMON BLOCK TRIGGERED! 🔥🔥🔥');
      debugPrint('Current Score: ${gameState.score}');
      debugPrint('Next piece will be: DEMON (10 cells, GOLD color)');
      debugPrint('═══════════════════════════════════════════');
      debugPrint('');

      // 創建一個單次惡魔方塊提供器並推入堆疊
      gameState.pieceProviderStack.push(_DemonBlockProvider());

      // 更新預覽隊列
      gameState.updatePreviewQueue();

      debugPrint(
          '[GameLogic] Demon block queued! Next piece will be a demon block.');
    }
  }
}

/// 臨時惡魔方塊提供器（用於單次惡魔方塊生成）
class _DemonBlockProvider implements IPieceProvider {
  bool _used = false;
  ...
  @override
  bool get isExhausted => _used;

  @override
  int get priority => 1; // 高優先級

  @override
  String get description => 'DemonBlockProvider(used: $_used)';
}
